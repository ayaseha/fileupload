...
public class FileUploadUtil {
	...
	public static String s3TempToSrc(AmazonS3Client s3Client, String bucket, String bucketKey, String uploadKey) {
		CopyObjectRequest cor=new CopyObjectRequest(bucket, bucketKey, bucket, uploadKey);
		s3Client.copyObject(cor.withCannedAccessControlList(CannedAccessControlList.PublicRead));
		return s3Client.getUrl(bucket, bucketKey).toString().substring(6);
	}
